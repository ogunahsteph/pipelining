...
def get_client(client_bvn:str, govt_companies: pd.DataFrame, multinational_companies: pd.DataFrame) -> pd.DataFrame:
    client = remita_hook.get_pandas_df(
        sql=f"""
            select Bvn, last_salary_within_9_months, last_salary_within_6_months, is_govt_employee, 
            IFNULL(is_mnc_employee, FALSE) AS is_mnc_employee from remita_staging.Customers c 
            left join (
                select BvnNo, 
...
def create_salaries(df, new_bvns, salaries_bvns, default_date):
    ...
    for x in new_bvns:
        if x in salaries_bvns:
            ...
            df.loc[len(df), df_cols] = [1, '2341111111', x, default_date, default_date, 0, '67676767', '000',
                                        'dummy_data', 0, False]
...
def government_employees_salaries_check(salaries, max_month, default_date):
    ...
    monthly_salary_summaries_last_6_months = \
    last_6_months.groupby(['BvnNo', 'year_month_salary_date'])['Amount'].sum().reset_index().groupby('BvnNo')[
        'Amount'].mean().rename('average_monthly_salary_last_6_months').reset_index()
    ...
    if (last_9_months.empty) & (last_6_months.empty):
        ...
        last_9_months_salaries_summaries[salaries_summaries_cols] = [0, 0, 0, default_date, default_date, 0, 0,
        ...
        last_9_months_salaries_summaries[salaries_summaries_cols] = [0, 0, 0, default_date, default_date, 0,
                                                                     monthly_salary_summaries_last_6_months[
                                                                         'average_monthly_salary_last_6_months'].values[
                                                                         0],
                                                                     salaries['BvnNo'].values[0], 0, False, 1]
        return last_9_months_salaries_summaries
    last_9_months = last_9_months.drop_duplicates(subset='payment_dates')
    ...
    monthly_salary_summaries = monthly_salary_summaries.merge(monthly_salary_summaries_last_6_months, on='BvnNo',
                                                              how='outer')

    last_9_months_salaries_summaries = last_9_months.groupby(['BvnNo']).agg(
        count_of_payments_last_9_months=pd.NamedAgg('payment_dates', 'nunique'),
...
def non_government_employees_salaries_check(salaries, max_month, default_date):
    ...
    if last_6_months.empty:
        ...
        salaries_summaries_cols = ['count_of_payments_last_6_months',
                                   ...
                                   'is_qualified_on_salaries', 'is_govt_employee']
        last_6_months_salaries_summaries[salaries_summaries_cols] = [0, 0, 0, default_date, default_date, 0,
                                                                     salaries['BvnNo'].values[0], 0, False, 0]

        return last_6_months_salaries_summaries

    last_6_months = last_6_months.drop_duplicates(subset='payment_dates')
    ...
    last_6_months_salaries_summaries = last_6_months.groupby(['BvnNo']).agg(
        count_of_payments_last_6_months=pd.NamedAgg('payment_dates', 'nunique'),
...
def government_employees_salaries_check_30_day_product(salaries, max_month):
    ...
    last_3_months = salaries[salaries['offset_payment_dates'] >= max_month + relativedelta(
        months=-2)]
    last_3_months = last_3_months.drop_duplicates(subset='payment_dates')
    ...
    last_3_months_salaries_summaries = last_3_months.groupby(['BvnNo']).agg(
        count_of_payments_last_3_months=pd.NamedAgg('payment_dates', 'nunique'),
    ...
    if last_3_months_salaries_summaries.empty:
        ...
        last_3_months_salaries_summaries[salaries_summaries_cols] = [salaries['BvnNo'].values[0], 0, 0, 0, default_date,
                                                                     default_date, 0, 0, False, False]
...
def calculate_additional_summaries(salaries_summaries: pd.DataFrame, loans: pd.DataFrame, time_period: int, salaries_summaries_30_day:pd.DataFrame) -> pd.DataFrame:
    ...
    salaries_summaries_30_day.rename(columns={'is_qualified_on_salaries': 'pay_day_is_qualified_on_salaries'},
                                     inplace=True)
    ...
    # summaries['latest_sal_ge_min_payment_pay_day'] = summaries['latest_salary_payment_amount_last_3_months'] >= \
    #                                          summaries[
    #                                              'minimum_salary_payment_last_3_months'] or summaries['latest_salary_payment_amount_last_3_months'] >= summaries['average_monthly_salary_last_3_months']

    #     summaries['latest_sal_ge_min_payment_pay_day'] = np.where(summaries['latest_salary_payment_amount_last_3_months'] >= summaries['minimum_salary_payment_last_3_months'] | summaries['latest_salary_payment_amount_last_3_months'] >= summaries['average_monthly_salary_last_3_months']
    # , True, False)

    summaries['latest_sal_ge_min_payment_pay_day'] = np.where(
        (summaries['latest_salary_payment_amount_last_3_months'].fillna(0) >= summaries[
            'minimum_salary_payment_last_3_months'].fillna(0)) |
        (summaries['latest_salary_payment_amount_last_3_months'].fillna(0) >= summaries[
            'average_monthly_salary_last_3_months'].fillna(0)),
        True,
        False
    )
...
def score_client(client_bvn: str, govt_companies: pd.DataFrame, multinational_companies: pd.DataFrame) -> pd.DataFrame:
    ...
    for x in no_summaries:
        summaries.loc[len(summaries), summaries_cols] = [
            x, 0, 0, 0, 0, 0, 0, default_date, default_date, default_date, default_date, 0, 0, 0, 0, False, False, 0, 0,
            0, 0, False, False, 0, 0, 0, 1 if time_period == 9 else 0, False
        ]
...
def rules_summary_narration(df):
    ...
    elif (final_allocated_limit_6_months > 0 or final_allocated_limit_3_months > 0) and (
                    final_is_qualified_6_months == True or final_is_qualified_3_months == True):
...
def get_3_months6_months_loans_data(bvn) -> int:
...
def get_scoring_results(bvn):
    multinational_companies = warehouse_hook.get_pandas_df(
        sql="""select business_name from remita.company_dimension where is_multinational"""
    ...
    client = get_client(client_bvn=bvn, govt_companies=govt_companies, multinational_companies=multinational_companies)
    ...
    extra = {'limit_reason': '', 'payday_limit_reason': '', 'final_is_qualified': False,
             'payday_final_is_qualified': False, 'is_govt_employee': str(is_govt_employee),
             'is_mnc_employee': str(is_mnc_employee)}
    if not client.empty:
        if bool(client.iloc[0]['last_salary_within_9_months']) or bool(client.iloc[0]['last_salary_within_6_months']):
            ...
            loans_data_3_months_6_months=get_3_months6_months_loans_data(bvn)
            added_6_months_rules=additional_6_months_rules(loans_data_3_months_6_months)

            # Additional CRB Logic
            crb_data_results = get_additional_crb_logic(client_bvn=bvn, scoring_summaries=summaries)['crb_data_results']
            ...
            final_df['final_is_qualified_3_months'] = False
            ...
            final_df[["rules_summary_narration", "limit_reason"]] = final_df["rules_summary_narration"].astype(
                "str").str.split(":", expand=True)

            if extra['is_govt_employee'] == '1' or extra['is_mnc_employee']:
                final_df['payday_rules_summary_narration'] = final_df.apply(lambda x: payday_rules_summary_narration(x),
                                                                            axis=1)
                final_df[["payday_rules_summary_narration", "payday_limit_reason"]] = final_df[
                    "payday_rules_summary_narration"].astype("str").str.split(":", expand=True)
            else:
