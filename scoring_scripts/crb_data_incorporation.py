...
def get_deposit_money_banks():
    DMBs_list = warehouse_hook.get_pandas_df(
        sql="""select instituition_name  from remita.deposit_money_banks"""
    )['instituition_name'].tolist()
...
def get_additional_crb_logic(client_bvn: str, scoring_summaries: pd.DataFrame) -> dict:
    ...
    DMBs_list = get_deposit_money_banks()
    ...
    final_df.loc[(final_df['total_no_of_running_facilities'] <= 5) &
                 ...
                 (final_df['total_overdue_amount'] < scoring_summaries['average_monthly_salary_last_3_months']) &
