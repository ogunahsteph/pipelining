...
with DAG(
    ...
    def push_mobile_credit_data(loan, product, mifos_tenant: str):
        ...
        if not amounts_exception.empty:
            file_name = f'CRB-{str(product).title()}-Exception-Amounts-Less-1000-{str(current_date)}.xlsx'
    ...
    def push_credit_data(product):
        ...
        if product == "bloom1":
            ...
            # get loans closed on mifos and loans with saf balances
            loan_result = warehouse_hook.get_pandas_df("""
                                    SELECT * FROM bloomlive.loans_fact_table_materialized_summary_view 
                                    WHERE bloom_version = '2'
                                        AND loan_status IN (300, 600, 700)
                                        AND disbursed_on_date >= '2021-11-11'
                                        AND ((total_outstanding = 0 AND safaricom_loan_balance IS NULL)
                                        OR safaricom_loan_balance IS NOT NULL)""",
                                                       )
    ...
    def process_result(**context):
        ...
        for product in products:
            ...
            if not loan_info.empty:
                ...
                # drop saf client test account by id
                if product == 'bloom2':
                    loan_data = loan_data[loan_data['client_mifos_id'].astype(str) != '80011']
    ...
    notify_teams = MSTeamsWebhookOperator(
        ...
        message="""CRB Bloom2 Pipeline Update""",
        subtitle="""Bloom2 CRB files generated and uploaded to sharepoint""".strip(),
        dag=dag,
        retries=0
    )
