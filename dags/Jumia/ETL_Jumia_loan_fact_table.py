...
with DAG(
    ...
    dag.doc_md = """
    ####DAG SUMMARY
    Extracts data from MIFOS and adds to ubuntu.jumia.loans_fact_table following SCD type 2
    DAG is set to run daily at 5 am.

    #### Actions
    <ol>
    <li>Extract from mifostenant-tanda.m_loan</li>
    <li>Check for existing loan instances in ubuntu.jumia.loans_fact_table</li>
    <li>Update existing instances in ubuntu.jumia.loans_fact_table</li>
    <li>Insert latest loan details in ubuntu.jumia.loans_fact_table</li>
    </ol>
    """
    ...
    def extract_load_loans_fact_table(**context) -> None:
        ...
        :return: None
        """
        el_loans_fact_table(mifos_tenant='tanda', warehouse_schema='jumia')
