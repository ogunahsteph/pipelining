...
with DAG(
        ...
        tags=['extract_load'],
        description='Load data into eabl dimensions',
    ...
    #### Actions
    <ol>
    <li>Extract from mifostenant-tanda.m_staff load into ubuntu.obradleys_eabl.staff_dimension</li>
    <li>Extract from mifostenant-tanda.m_product_loan load into ubuntu.obradleys_eabl.product_dimension</li>
    <li>Extract from mifostenant-tanda.m_client load into ubuntu.obradleys_eabl.client_dimension</li>
    <li>Extract from mifostenant-tanda.r_enum_value load into ubuntu.obradleys_eabl.loan_type_dimension</li>
    </ol>
    """

    common_params = {
        'eabl_fund_id': '{{eabl_fund_id}}',
        'eabl_office_id': '{{eabl_office_id}}',
        'eabl_product_ids': default_args['eabl_product_ids']
    }
    ...
    def extract_load_transactions_dimension(**context):
        ...
        """
        mifos_tenant = 'tanda'
        warehouse_schema = 'obradleys_eabl'
        transaction_types = [1, 2]
        office_id = context['templates_dict']['eabl_office_id']
    ...
    def extract_load_client_dimension(**context) -> None:
        """
        Extracts from mifostenant-tanda.m_client and loads into ubuntu.obradleys_eabl.client_dimension
        """
        ...
        clients = get_mifos_clients(
            mifos_tenant=mifos_tenant,
            warehouse_schema=warehouse_schema,
            mifos_filters=f"office_id = {context['templates_dict']['eabl_office_id']}"
        )
    ...
    def extract_load_product_dimension(**context) -> None:
        ...
        all_products = get_mifos_products_for_dimension(
            ...
            mifos_filters=f"where id in {tuple(list(context['templates_dict']['eabl_product_ids']))}"
        )
    ...
    def trigger_ETL_EABL_loans_fact_table(**context):
        from utils.common import trigger_dag_remotely

        # Triggering the DAG remotely
        trigger_dag_remotely(dag_id='ETL_EABL_loans_fact_table', conf={})
    ...
    t5 = PythonOperator(
        task_id='trigger_ETL_EABL_loans_fact_table',
        provide_context=True,
        python_callable=trigger_ETL_EABL_loans_fact_table
    )
    t1 >> t2 >> t3 >> t4 >> t6 >> t5
