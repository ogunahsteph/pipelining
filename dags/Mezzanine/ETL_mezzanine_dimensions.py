...
default_args = {
    ...
    'mezzanine_office_id': 1,
    'mezzanine_product_ids': [1],
}

local_tz = pendulum.timezone("Africa/Nairobi")

with DAG(
        'ETL_mezzanine_dimensions',
        default_args=default_args,
        catchup=False,
        schedule_interval='0 */3 * * *',
        start_date=datetime.datetime(2022, 2, 22, 10, 00, tzinfo=local_tz),
        tags=['extract_load'],
        description='Load data into mezzanine dimensions',
    ...
    #### Actions
    <ol>
    <li>Extract from mifostenant-uganda.m_staff load into ubuntu.mezzanine.loan_officer_dimension</li>
    <li>Extract from mifostenant-uganda.m_product_loan load into ubuntu.mezzanine.product_dimension</li>
    <li>Extract from mifostenant-uganda.m_client load into ubuntu.mezzanine.client_dimension</li>
    <li>Extract from mifostenant-uganda.r_enum_value load into ubuntu.mezzanine.loan_status_dimension</li>
    </ol>
    """

    def extract_load_client_dimension(**context) -> None:
        """
        Extracts from mifostenant-uganda.m_client and loads into ubuntu.mezzanine.client_dimension
        """
        mifos_tenant = 'uganda'
        warehouse_schema = 'mezzanine'

        clients = get_mifos_clients(
            mifos_tenant=mifos_tenant,
            warehouse_schema=warehouse_schema,
            mifos_filters=f"office_id = {context['templates_dict']['mezzanine_office_id']}"
        )

        if clients is not None:
            # INSERT NEW RECORDS
            insert_new_records(
                table_name='mezzanine.client_dimension',
                target_fields=[
    ...
    def extract_load_product_dimension(**context) -> None:
        ...
        iterables = ', '.join("{0}".format(w) for w in list(context['templates_dict']['mezzanine_product_ids']))
    ...
    def extract_load_transactions_dimension(**context):
        ...
        office_id = context['templates_dict']['mezzanine_office_id']
    ...
    def extract_load_loan_officer_assignment_history_dimension(**context):
        """
        Extracts from mifostenant-uganda.m_payment_detail and loads into ubuntu.mezzanine.transactions_dimension
        """

        existing_records = warehouse_hook.get_pandas_df(
            sql="""
                SELECT mifos_id FROM mezzanine.loan_officer_assignment_history
            """
        )
        ...
        if assignment_history.shape[0] > 0:
            ...
            # INSERT NEW RECORDS
            insert_new_records(
                table_name='mezzanine.loan_officer_assignment_history',
                target_fields=[
    ...
    def trigger_ETL_mezzanine_loans_fact_table(**context):

        from utils.common import trigger_dag_remotely

        # Triggering the DAG remotely
        trigger_dag_remotely(dag_id='ETL_mezzanine_loans_fact_table', conf={})
    ...
    t1 = PythonOperator(
        ...
        templates_dict={
            'mezzanine_office_id': '{{mezzanine_office_id}}',
        },
    )
    t2 = PythonOperator(
    ...
    t3 = PythonOperator(
        ...
        templates_dict={
            'mezzanine_office_id': '{{mezzanine_office_id}}',
            'mezzanine_product_ids': default_args['mezzanine_product_ids']
        },
    ...
    t5 = PythonOperator(
        task_id='trigger_ETL_mezzanine_loans_fact_table',
        provide_context=True,
        python_callable=trigger_ETL_mezzanine_loans_fact_table
    )
