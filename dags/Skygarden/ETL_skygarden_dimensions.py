...
default_args = {
    ...
    'skygarden_office_id': 6,
    'skygarden_product_ids': [13,14],
...
with DAG(
        ...
        tags=['extract_load'],
        description='Load data into skygarden dimensions',
    ...
    #### Actions
    <ol>
    <li>Extract from mifostenant-tanda.m_staff load into ubuntu.skygarden.staff_dimension</li>
    <li>Extract from mifostenant-tanda.m_product_loan load into ubuntu.skygarden.product_dimension</li>
    <li>Extract from mifostenant-tanda.m_client load into ubuntu.skygarden.client_dimension</li>
    <li>Extract from mifostenant-tanda.r_enum_value load into ubuntu.skygarden.loan_type_dimension</li>
    </ol>
    """

    common_params = {
        'skygarden_office_id': '{{skygarden_office_id}}',
        'skygarden_product_ids': default_args['skygarden_product_ids']
    }
    ...
    def extract_load_transactions_dimension(**context):
        ...
        """
        mifos_tenant = 'tanda'
        warehouse_schema = 'skygarden'
        transaction_types = [1, 2]
        office_id = context['templates_dict']['skygarden_office_id']
    ...
    def extract_load_client_dimension(**context) -> None:
        """
        Extracts from mifostenant-tanda.m_client and loads into ubuntu.skygarden.client_dimension
        """
        mifos_tenant = 'tanda'
        warehouse_schema = 'skygarden'
        ...
        clients = get_mifos_clients(
            mifos_tenant=mifos_tenant,
            warehouse_schema=warehouse_schema,
            mifos_filters=f"office_id = {context['templates_dict']['skygarden_office_id']}"
        )

        if clients is not None:
            # INSERT NEW RECORDS
            insert_new_records(
                table_name='skygarden.client_dimension',
                target_fields=[
    ...
    def extract_load_product_dimension(**context) -> None:
        ...
        all_products = get_mifos_products_for_dimension(
            ...
            mifos_filters=f"where id in {tuple(list(context['templates_dict']['skygarden_product_ids']))}"
        )
    ...
    def trigger_ETL_skygarden_loans_fact_table(**context):
        from utils.common import trigger_dag_remotely

        # Triggering the DAG remotely
        trigger_dag_remotely(dag_id='ETL_skygarden_loans_fact_table', conf={})
    ...
    t5 = PythonOperator(
        task_id='trigger_ETL_skygarden_loans_fact_table',
        provide_context=True,
        python_callable=trigger_ETL_skygarden_loans_fact_table
    )
