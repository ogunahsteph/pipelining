...
from utils.common import on_failure, get_mifos_clients, product_dimension_process_new_products, el_transactions_dimension, el_staff_dimension, el_loan_status_dimension

warehouse_hook = PostgresHook(postgres_conn_id='rds_afsg_ds_prod_postgresql_dwh', schema='afsg_ds_prod_postgresql_dwh')
mifos_hook = MySqlHook(mysql_conn_id='mifos_db', schema='mifostenant-bloom1restructure')
...
default_args = {
    ...
    'bloom1_restructured_product_ids': [1, 2]
}

local_tz = pendulum.timezone("Africa/Nairobi")

with DAG(
        'ETL_bloom1_restructured_dimensions',
        default_args=default_args,
        catchup=False,
        schedule_interval='0 */3 * * *',
        start_date=datetime.datetime(2023, 6, 14, 9, 00, tzinfo=local_tz),
        tags=['extract_load'],
    ...
    #### Actions
    <ol>
    <li>Extract from mifostenant-bloom1restructure.m_staff load into ubuntu.bloomlive.staff_dimension</li>
    <li>Extract from mifostenant-bloom1restructure.m_product_loan load into ubuntu.bloomlive.product_dimension</li>
    <li>Extract from mifostenant-bloom1restructure.m_client load into ubuntu.bloomlive.client_dimension</li>
    <li>Extract from mifostenant-bloom1restructure.r_enum_value load into ubuntu.bloomlive.loan_type_dimension</li>
    </ol>
    
    To trigger dag with config
    {
        "month": 4, 
        "year": 2023
    }
    ...
    def extract_load_client_dimension(**context) -> None:
        ...
        # Extract data from mifostenant-bloom1restructure.m_client table
        clients = get_mifos_clients(
            mifos_tenant='bloom1restructure',
            warehouse_schema='bloomlive',
            warehouse_filters="bloom_version = 'bloom1_restructured'"
        )

        if clients is not None:
            clients['bloom_version'] = 'bloom1_restructured'
    ...
    def extract_load_product_dimension(**context) -> None:
        ...
        all_products = get_mifos_products_for_dimension(
            ...
            mifos_filters=f"where id in {tuple(list(context['templates_dict']['bloom1_restructured_product_ids']))}"
        )
        ...
        if len(changed_records) > 0:
            ...
            warehouse_hook.run(
                sql=f"update bloomlive.product_dimension set bloom_version = 'bloom1_restructured' where surrogate_id in ({iterables})")

        products['bloom_version'] = 'bloom1_restructured'
    ...
    def extract_load_staff_dimension(**context) -> None:
        ...
        el_staff_dimension(
            warehouse_schema=warehouse_schema,
            warehouse_filters="where bloom_version = 'bloom1_restructured'",
            mifos_hook=MySqlHook(mysql_conn_id='mifos_db', schema=f'mifostenant-{mifos_tenant}')
        )
    ...
    def trigger_ETL_bloom1_restructured_loans_fact_table(**context):
        """
        ...
        ** context: Additional context to be passed to the DAG when triggered.
        ...
        trigger_dag_remotely(dag_id='ETL_bloom1_restructured_loans_fact_table', conf={})
    ...
    def extract_load_loan_officer_assignment_history_dimension(**context):
        ...
        assignment_history = get_mifos_loan_officer_assignment_history(
            mifos_tenant='bloom1restructure',
            warehouse_schema='bloomlive',
            filters="WHERE bloom_version = 'bloom1_restructured'",
            warehouse_hook=PostgresHook(postgres_conn_id='rds_afsg_ds_prod_postgresql_dwh', schema='afsg_ds_prod_postgresql_dwh'),
            mifos_hook=MySqlHook(mysql_conn_id='mifos_db', schema=f'mifostenant-bloom1restructure'),
        )

        if assignment_history.shape[0] > 0:
            # set the bloom version
            assignment_history['bloom_version'] = 'bloom1_restructured'
    ...
    common_params = {'bloom1_restructured_product_ids': default_args['bloom1_restructured_product_ids']}
    ...
    t5 = PythonOperator(
        ...
        provide_context=True,
        python_callable=trigger_ETL_bloom1_restructured_loans_fact_table
    )
